...
## ----filepath1----------------------------------------------------------------------------------------------------------
inputDat <- file.path("Outputs") # Define file paths
...
## -----------------------------------------------------------------------------------------------------------------------
PUs <- readRDS(file.path(inputDat, "RDS/PUs", "PUs.rds")) %>%
  st_transform(cCRS) %>%
  rowid_to_column( var= "cellID")


## -----------------------------------------------------------------------------------------------------------------------
features <- readRDS(file.path(inputDat, "RDS/PUs", "PUs_all_features_cost.rds")) %>% 
  st_transform(cCRS) %>%
  rowid_to_column( var= "cellID") %>% 
  select(-cost)
...
ui <- page_sidebar(
  ...
  layout_columns(
    card(card_header("Planning area with selected areas for conservation prioritization", class = "h6 text-success"),
...
server <- function(input, output, session) {
  ...
  soln <- eventReactive(input$simulate,{
    problem() %>%
      solve.ConservationProblem(force = TRUE)
  })
...
# install.packages('C:\\gurobi1301\\win64\\R\\gurobi_13.0-1.zip', repos=NULL)
# 
